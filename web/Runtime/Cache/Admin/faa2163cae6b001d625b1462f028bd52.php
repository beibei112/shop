...
<!-- 管理员列表 -->
                    ...
                    <tbody role="alert" aria-live="polite" aria-relevant="all">
                        <?php if(is_array($list)): foreach($list as $k=>$vo): if($k%2 == 0): ?><tr class="odd">
                            <?php else: ?>
                                ...
                                <td class="  "><?php echo getUser($vo['user_id']);?></td>
                               
                                <td class="  "><?php echo getAddress($vo['address_id']);?></td>
                                <td class="  ">
                                     <button type="button" class="btn dropdown-toggle" data-toggle="dropdown">
                                            $<?php echo ($vo['total']); ?><span class="caret"></span>
                                        </button>

                                        <ul class="dropdown-menu pull-left" style='position:absolute;top:125px;left:590px;'>
                                            <?php if(is_array($vo["good"])): foreach($vo["good"] as $key=>$v): ?><li>
                                                    <a href="#">
                                                        <img src="<?php echo ($v['path']); ?>" width="40px">
                                                        $<?php echo ($v['price']); ?>
                                                        <?php echo ($v['num']); ?>件
                                                    </a>
                                ...
                                <td class="  "><?php echo orderStatus($vo['status']);?></td>
                                                     
                                <td class=" " width='15%'>
                                    <button class='btn btn-danger btn-small' onclick='fun(this)' info='<?php echo ($vo['id']); ?>'>
                                        <?php if($vo["status"] == 0): ?>发货
                                        <?php else: ?>
                                            已发货<?php endif; ?>
                                    </button>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function fun(obj){
            ...
            $.ajax({
                url:'/index.php/Admin/Order/changeStatus',
                data:{id:id},
